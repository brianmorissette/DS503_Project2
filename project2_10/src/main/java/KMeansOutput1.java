...
public class KMeansOutput1 {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 5) {
            System.err.println("Usage: KMeansOutput1 <k> <R> <threshold> <input path> <output path>");
            System.exit(-1);
        }
        ...
        for (int iter = 0; iter < R; iter++) {
            ...
            job.setJarByClass(KMeansOutput1.class);
        ...
        // Reorganize output: move centroids to dedicated folder, write convergence indicator
        Path finalIterPath = new Path(outputPath + "/iter_" + lastIter);
        Path centroidsPath = new Path(outputPath + "/centroids");
        Path convergencePath = new Path(outputPath + "/convergence.txt");

        fs.mkdirs(centroidsPath);
        org.apache.hadoop.fs.FileStatus[] partFiles = fs.listStatus(finalIterPath,
                path -> path.getName().startsWith("part-r-"));
        for (org.apache.hadoop.fs.FileStatus f : partFiles) {
            fs.rename(f.getPath(), new Path(centroidsPath, f.getPath().getName()));
        }

        try (org.apache.hadoop.fs.FSDataOutputStream out = fs.create(convergencePath)) {
